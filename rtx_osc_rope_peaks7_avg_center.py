...
pts = np.array([[int(dimensions[1]*0.01),int(dimensions[0]*0.2)],
                [int(dimensions[1]*0.95),int(dimensions[0]*0.01)],
                [int(dimensions[1]*0.95),int(dimensions[0]*0.8)],
                [int(dimensions[1]*0.01),int(dimensions[0]*0.7)]], np.int32)
...
peak_ids += empty_id
prev_peak_ids = np.zeros(max_n_peaks)
prev_peak_ids += empty_id
current_peak_id = 0
peak_max_change = 300
...
def peak_id_following(peak_indices, peak_ids, prev_peak_ids, current_peak_id, peak_max_change, max_n_peaks, empty_id, mask_left, mask_right):
    '''
    ...
    new_ids = []
    continued_ids = []
    min_peak_distance = 3 # ad hoc, filtering of double peaks (too close)
    for p in peak_indices:
        current_peak_id = current_peak_id % max_n_peaks
        if np.min(prev_peak_ids) == empty_id: # if no previous ids, just write new ones
            ...
            if abs(p-prev_peak_ids[index]) < peak_max_change: 
                if abs(p-prev_peak_ids[index]) > min_peak_distance:
...
try:
    ...
    while True:
        ...
        noise_gate_diff = np.sum(np.abs(wave_1D-prev_wave_1D))
        ...
        if noise_gate > 0:
            ...
            peak_indices = []
        current_peak_id, peak_ids, active_ids, deleted_ids = peak_id_following(peak_indices, peak_ids, prev_peak_ids, current_peak_id, peak_max_change, max_n_peaks, empty_id, mask_left, mask_right)
        prev_peak_ids = np.copy(peak_ids)
        
        #if len(active_ids)+len(deleted_ids) > 0:
        #    print('peaks', peak_ids, active_ids)
        ...
        if numpeaks > 0 :
            ...
            y_prev = 0
            for i in range(len(active_ids_sorted)):
                peak_id, x, x_movement = active_ids_sorted[i]
                sum_x_movement += x_movement
                #print(f'movement: peak{peak_id} x {x}, x_mov {x_movement}, sum {sum_x_movement}, active_id {active_ids_sorted[i]}')
                active_ids_sorted[i][2] = x # write previous x position to array
            ...
            avg_x_movement = sum_x_movement/numpeaks
            
        # send peaks summary to Csound
        #print('sum_x_movement', sum_x_movement)
        osc_msg = numpeaks, float(avg_x_distance), float(avg_x_movement), float(sum_x_movement)
