<Cabbage>
form size(900, 510), caption("Stats rope"), pluginId("sr11"), guiMode("queue"), colour(30,30,30)
...
rslider channel("graincloud_ampthresh"), bounds(10, 230, 60, 60), text("M.a_thresh"), range(-90, 0, -5)
nslider channel("graincloud_transpose"), bounds(70, 230, 50, 30), text("M.transp"), range(-24, 24, 0, 1, 1)
nslider channel("graincloud_midichan"),  bounds(70, 260, 50, 30), text("M.chan"), range(1, 16, 1, 1, 1)
...
opcode Graincloud, aa, kkkkkkkkkkkiii
  kamp, kwavfreq, kpitchmod, kpitch_spread, kgrainrate, kratemod, kdistribution, kgraindur, kamp_thresh, ktranspose, kchan, ivoice, imaxvoice, iopcode_id xin
...
; system parameter
  ...
  ;puts Sdebug, kamp+kpulse+1
  kphase downsamp aphase
  if (kpulse > 0) && (kamp > kamp_thresh) && ivoice < 3 then
    knote = (kwavfreq*12)+48
    knote = 12*log2(kwavfreq/440) + 69 + ktranspose
    kvel limit kamp*120, 40, 127
    kmidi_chan chnget "distgrains_midichan"
    event "i", 202, 0, (kduration/1000)+0.01, kvel, knote, kmidi_chan+ivoice
  endif

  if (ivoice < imaxvoice-1) then
    a1a, a2a Graincloud kamp, kwavfreq, kpitchmod, kpitch_spread, kgrainrate, kratemod, kdistribution, kgraindur, kamp_thresh, ktranspose, kchan, ivoice+1, imaxvoice, iopcode_id
    a1 += a1a
    a2 += a2a
  endif
...
instr 13
  ...
  ; midi
  kmidi_amp_thresh chnget "graincloud_ampthresh"
  kmidi_amp_thresh ampdbfs kmidi_amp_thresh
  kmidi_transpose chnget "graincloud_transpose"
  kmidi_chan chnget "graincloud_midichan"
  iopcode_id = 20

  a1,a2 Graincloud kamp, kwavfreq, kpitchmod, kpitch_spread, kgrainrate, kratemod, kdistribution, kgraindur, kmidi_amp_thresh, kmidi_transpose, kmidi_chan, 0, imaxvoice, iopcode_id
