...
# flier padding of wave_1D
filter_padding = 20
wave_1D = np.zeros(mask_xrange+(filter_padding*2))
...
def fill_in_missing_points(y_init, input_1D, output_1D, output_img, show_fill_blanks, fill_blanks_color):
    ...
    #line_len = mask_xrange-x_prev
    #line = np.linspace(y_prev, y_init, line_len)
    #output_1D[x_prev:mask_xrange] = np.reshape(line, shape=(line_len,))
    if show_fill_blanks:
        for i in range(mask_xrange):
...
try:
    ...
    while True:
        ...
        fader_distance = int(((mask_xrange)-fader_pad*2)/num_faders)
        for i in range(num_faders):
            #faders[i] = wave_1D[i*fader_distance+fader_pad+mask_left]#/max_amp
            faders[i] = wave_1D[i*int(mask_xrange/num_faders)]#/max_amp
        #x = (i*fader_distance)+mask_left+fader_pad
