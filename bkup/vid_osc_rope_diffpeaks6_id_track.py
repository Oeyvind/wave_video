...
peaks_pos_prev = np.zeros(1)
peaks_neg_prev = np.zeros(1)
...
try:
    ...
    while True:
        ...
        # find peaks new
        # diff of wave
        # peak index is at sign change
        wave_1D_diff = np.diff(wave_1D)
        wave_1D_diff_sign = np.sign(wave_1D_diff)
        sign = 1
        # get rid of zeroes
        for i in range(len(wave_1D_diff_sign)):
            if wave_1D_diff_sign[i] == 0: 
                wave_1D_diff_sign[i] = sign
            sign = wave_1D_diff_sign[i]
        peaks = np.diff(wave_1D_diff_sign)
        peaks_pos = np.where(np.greater(peaks,0))[0]
        peaks_neg = np.where(np.less(peaks,0))[0]
        print('peaks_pos', peaks_pos)
        #print('peaks_neg', peaks_neg)
        
        contin_peak_threshold = 50
        contin_peaks = []
        new_peaks = []
        for p in peaks_pos:
            ppos = np.argmin(np.abs(peaks_pos_prev-p))
            print('the', peaks_pos_prev[ppos])
            if peaks_pos_prev[ppos] - p < contin_peak_threshold:
                contin_peaks.append(int(p))
            else:
                new_peaks.append(int(p))
        if np.size(peaks_pos) > 0:
            peaks_pos_prev = np.copy(peaks_pos)
            #print('*prev*', peaks_pos_prev)
        print('new', new_peaks)
        print('cont', contin_peaks)
        ...
        for i in range(len(wave_1D_diff_sign)):
            if show_peaks:
                ...
                if np.size(peaks_pos) > 0: 
                    if i in peaks_pos:
                        cv2.circle(wave_img, (i+offset,y),15, peakplus_color, 8)
                        #cv2.putText(wave_img, f'{peak_id}', (i-20,y+45), cv2.FONT_HERSHEY_SIMPLEX, 1, peakplus_color, 2, cv2.LINE_AA)
                if np.size(peaks_neg) > 0: 
                    if i in peaks_neg:
                        cv2.circle(wave_img, (i+offset,y),15, peaknegative_color, 8)
                        #cv2.putText(wave_img, f'{peak_id}', (p-20,y-25), cv2.FONT_HERSHEY_SIMPLEX, 1, peaknegative_color, 2, cv2.LINE_AA)
        ...
        #        peak_id_list.append(i)
        
        #i = 0
        #print('peak_indices', peak_indices, 'pid_list', peak_id_list)
        for p in peak_indices:
            y = int(wave_1D[p])
            #print('peak',p,y)
            peak_id = p#peak_id_list[i]
            #i += 1
            if show_peaks:
                if y > wave_center[p]:
                    cv2.circle(wave_img, (p,y),15, peakplus_color, 8)
                    cv2.putText(wave_img, f'{peak_id}', (p-20,y+45), cv2.FONT_HERSHEY_SIMPLEX, 1, peakplus_color, 2, cv2.LINE_AA)
                else:
                    cv2.circle(wave_img, (p,y),15, peaknegative_color, 8)
                    cv2.putText(wave_img, f'{peak_id}', (p-20,y-25), cv2.FONT_HERSHEY_SIMPLEX, 1, peaknegative_color, 2, cv2.LINE_AA)
