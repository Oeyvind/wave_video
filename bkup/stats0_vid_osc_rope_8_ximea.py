...
def display_faders(faders, num_faders, wave_size, mask_left, output_img, show_faders, fader_color):
    for i in range(num_faders):
        y = int(faders[i])
        x = i*int(wave_size/num_faders)+mask_left
        if show_faders:
            cv2.circle(output_img, (x,y), 15, fader_color, 8)
            cv2.putText(output_img, f'{y}', (x-20,y+45), cv2.FONT_HERSHEY_SIMPLEX, 1, fader_color, 2, cv2.LINE_AA)
...
try:
    ...
    while True:
        ...
        if np.sum(frame_diff) < diff_thresh:
            ...
            for i in range(num_faders):
                faders[i] = wave_1D[i*int(dimensions[1]/num_faders)]
            display_faders(faders, num_faders, dimensions[1], mask_left, wave_img, show_faders, fader_color)
            ...
            # other stats
            x_pos = active_ids_sorted[:,1]
            x_distances = np.diff(active_ids_sorted[:,1])
            ...
            if show_stats:
                ...
                legend_y += v_offset*2
                cv2.putText(output, f'avg_x_dist: {avg_x_distance}, avg movement {avg_x_movement}', (legend_x+15,legend_y+5), cv2.FONT_HERSHEY_SIMPLEX, 1, stats_color, 1, cv2.LINE_AA)
                legend_y += v_offset*2
                x_pos_disp = 'x_pos :'
                for x in x_pos:
                    x_pos_disp = x_pos_disp + str(x) + ', '
                ...
                for x in x_distances:
                    x_dist_disp = x_dist_disp + str(x) + ', '
                cv2.putText(output, x_dist_disp, (legend_x+15,legend_y+5), cv2.FONT_HERSHEY_SIMPLEX, 1, stats_color, 1, cv2.LINE_AA)
                legend_y += v_offset*2
                cv2.putText(output, f'zero_cross: {zero_crossings[0]}', (legend_x+15,legend_y+5), cv2.FONT_HERSHEY_SIMPLEX, 1, stats_color, 1, cv2.LINE_AA)
                legend_y += v_offset*2
                zc_disp = 'zc_dist: '
                for i in zc_diff:
                    zc_disp = zc_disp + str(i) + ', '
