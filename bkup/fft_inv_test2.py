...
cv2.circle(img1, (int(dimensions[1]/8), int(dimensions[0]/8)), 1, 255, pix_size)
img2 = np.zeros(size,dtype='uint8')
cv2.circle(img2, (int(dimensions[1]/16), int(dimensions[0]/16)), 1, 255, pix_size)
...
img_back1z = cv2.resize(img_back1[0:int((size_y/zoom_factor)),
                                  0:int((size_x/zoom_factor))],
                                  None, fx=zoom_factor, fy=zoom_factor)
img_back2z = cv2.resize(img_back2[0:int((size_y/zoom_factor)),
                                  0:int((size_x/zoom_factor))],
                                  None, fx=zoom_factor, fy=zoom_factor)
img_back3z = cv2.resize(img_back3[0:int((size_y/zoom_factor)),
                                  0:int((size_x/zoom_factor))],
                                  None, fx=zoom_factor, fy=zoom_factor)
