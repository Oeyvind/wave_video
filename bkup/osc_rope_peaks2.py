...
while(cap.isOpened()):
    ...
    # find peaks
    diff = np.diff(wave_1D)
    sign = np.sign(diff)
    diffsign = np.diff(sign)
    peaks = diff[1:]*np.abs(diffsign)
    thresh = 0.005
    offset = -50
    for i in range(200,dimensions[1]):
        p = peaks[i]
        if p > thresh:
            y = int(wave_1D[i])
            #print(i,y)
            cv2.circle(wave_img_col, (i+offset,y),15, (0,0,255), 8)
        if p < -thresh:
            y = int(wave_1D[i])
            #print(i,y)
            cv2.circle(wave_img_col, (i+offset,y),15, (0,255,0), 8)
