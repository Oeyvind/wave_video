...
pts = np.array([[10,50],[630,100],[630,470],[10,300]], np.int32)
...
while(cap.isOpened()):
    ...
    frame_diff_masked = cv2.blur(frame_diff_masked, (3,3))
    ...
    binary_masked = np.copy(binary_img)
    ...
    for i in range(dimensions[1]):
        ...
        if len(y_coords[0]) > 0:
            ...
            cv2.circle(binary_img, (i,y_centroid),2, (255), 1)# display sign
    ...
    for i in range(left_pos,dimensions[1]):
        new = np.zeros(dimensions[0])
    ...
    binary_img_color[:, :, 0] = 0
    binary_img_color[:, :, 2] = 0
    binary_masked_color = cv2.cvtColor(binary_masked, cv2.COLOR_GRAY2BGR)
    binary_masked_color[:, :, 1] = 0
    binary_masked_color[:, :, 2] = 0
    output = cv2.add(binary_img_color, binary_masked_color)

    #output = binary_img_color
    #output = cv2.add(wave_img_col, frame_col)
    #output = current_frame
    if show_mask:
        polyg_show = cv2.polylines(output,pts=[pts],isClosed=True, color=(255,0,0),thickness=2)
    #size = (640,480) # (300,200)
    #output = cv2.resize(output), size)
    cv2.imshow("Rope", output)

    # timing, frame rate
    #timenow = time.time()
    #print('time between frames:', timenow-timethen)
    #timethen = timenow
