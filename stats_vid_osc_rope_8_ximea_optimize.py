...
# potential kludge: 
#    - ranges for display of process data onto output size image
#    - resample severale times?
#   - create output frame in output size (resize current frame)
#   - then display onto this, scaling the mask values and offsets according to process_size/display_size
...
# check that median and lowpass filters process the correct range
...
print('size_calc', input_size, process_size, display_size)
...
def centroid_1D_from_img(input_img, output_img, show_centroid, centroid_color):
    # centroid
    size = np.shape(input_img)[1]
    centroid_1D = np.zeros(size)
    for i in range(size):
...
def median_1D(input_1D, filter_padding, filter_size1, filter_size2, output_img, show_medianfilter, medianfilter_color):
    # median filtering
    datasize = np.shape(input_1D)[0]-(filter_padding*2)
    for i in range(datasize+filter_padding-filter_size1):
        input_1D[i+int(filter_size1/2)] = np.median(input_1D[i:i+filter_size1])
    for i in range(datasize+filter_padding-filter_size2):
        input_1D[i+int(filter_size2/2)]= np.median(input_1D[i:i+filter_size2])    
    if show_medianfilter:
        for i in range(mask_left,datasize):
...
def lowpass_1D(input_1D, output_img, filter_padding, filter_size1, filter_size2, show_lowpassfilter, lowpass_color):
    # lowpass
    datasize = np.shape(input_1D)[0]-(filter_padding*2)
    for i in range(datasize+(filter_padding*2)-filter_size1):
        input_1D[i+int(filter_size1/2)] = np.mean(input_1D[i:i+filter_size1])
    for i in range(datasize+(filter_padding*2)-filter_size2):
...
try:
    ...
    while True:
        ...
        if activation_sum < diff_thresh:
            binary_img *= 0        
        #print('processing')
        binary_img_proc = cv2.resize(binary_img, process_size)
        # output img for display of curves
        wave_img = np.zeros((process_size[0], process_size[1],3), np.uint8)
        # find centroid, disambiguation of rope trace
        centroid_1D = centroid_1D_from_img(binary_img_proc, wave_img, show_centroid, centroid_color)
        ...
        # Display result
        #print('display', np.shape(current_frame), np.shape(wave_img))
        wave_img = cv2.resize(wave_img, display_size)
        current_frame_display = cv2.resize(current_frame, display_size)
        #print('resize', np.shape(wave_img))
        output = cv2.add(current_frame_display, wave_img)    
        if show_binary:
            #binary_img_bgr = cv2.cvtColor(binary_img, cv2.COLOR_GRAY2BGR)
            binary_img = cv2.resize(binary_img, display_size)
        ...
        if show_mask:
            display_pts = ((np.array(pts)*100*display_scale)/100).astype('int32').tolist()
            print(f'displaypts: {display_pts}, \n {pts} \n {type(pts[0][0][0]), {type(display_pts[0][0][0])}}')
            polyg_show = cv2.polylines(output,pts=[display_pts],isClosed=True, color=(255,0,0),thickness=2)
