...
try:
    ...
    while True:
        ...
        for i in range(len(active_ids)):
            active_ids[i].append(active_ids[i][1]) # duplicate the x position, we'll use this slot to write the previous x value
        ...
        if numpeaks > 0 :
            ...
            for i in range(len(active_ids_sorted)):
                peak_id, x, x_prev = active_ids_sorted[i]
                x_movement = x-x_prev
                sum_x_movement += x_movement
                print(f'movement: peak{peak_id} x {x}, x_pre {x_prev}, sum {sum_x_movement}, active_id {active_ids_sorted[i]}')
        ...
        osc_msg = numpeaks, avg_x_distance, float(avg_x_movement), float(sum_x_movement)
