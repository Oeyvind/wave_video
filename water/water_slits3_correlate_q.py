...
while(cap.isOpened()):
    ...
    frame_diff_filt = screen_blend_self(frame_diff_filt)
    ...
    # correlation to prev frame
    corr_thresh = 50
    corr = np.correlate(np.where(slit<corr_thresh,0,corr_thresh), np.where(prev_slit<corr_thresh,0,corr_thresh), mode='same')
    #corr = np.correlate(slit,prev_slit, mode='same')
    cposes = np.argsort(corr)
    csame = size[1]//2
    print('cposes', cposes[-30:]-csame)
    prev_slit = slit
    ...
    if show_fft:
        ...
        legend_y += 25
        cv2.putText(output_bgr, f'centr_Mid: {centr2:.2f} amp {centr2_maxamp:.2f} cpos {cposes[-1]-csame} {cposes[-2]-csame} {cposes[-3]-csame}', (legend_x,legend_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, txt_color, 1, cv2.LINE_AA)
        legend_y += 25
    ...
    if show_wave:
        ...
        for i in range(len(slit)):
            slit_clip = np.where(slit<corr_thresh,0,corr_thresh)
            cv2.circle(output_bgr, (i,int(y_pos-3-((slit_clip[i]/255)*wave_height))),1, wave_color, 1)# display wave
    if send_wave:
        for i in range(len(slit)):
