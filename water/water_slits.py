...
while(cap.isOpened()):
    ...
    y_first = 0.4
    y_offset = 0.115
    wave_height = size[0]* 0.08
    y_pos1 = int(size[0]* y_first)
    y_pos2 = int(size[0]* (y_first+(y_offset)))
    y_pos3 = int(size[0]* (y_first+(y_offset*2)))
    y_pos4 = int(size[0]* (y_first+(y_offset*3)))
    y_pos5 = int(size[0]* (y_first+(y_offset*4)))
    y_pos6 = int(size[0]* (y_first+(y_offset*5)))
    y_poses = [y_pos1,y_pos2,y_pos3,y_pos4,y_pos5,y_pos6]
    slits = np.zeros((6, size[1]))
    for i in range(6):
        slits[i] = output[y_poses[i],:]
    ...
    if show_fft:
        ...
        legend_y += 25
        cv2.putText(output_bgr, f'centr_Mid: {centr2:.2f} amp {centr2_maxamp:.2f}', (legend_x,legend_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, txt_color, 1, cv2.LINE_AA)
        legend_y += 25
