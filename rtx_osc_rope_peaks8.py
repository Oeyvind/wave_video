...
try:
    ...
    while True:
        ...
        if numpeaks > 0 :
            ...
            #print('active_ids_sorted\n', active_ids_sorted)
            if numpeaks > 1:
                avg_x_distance = (active_ids_sorted[-1][1]-active_ids_sorted[0][1])/(numpeaks-1)
            else: 
                avg_x_distance = active_ids_sorted[0][1]

            sum_x_movement = np.sum(active_ids_a[:,2])
            avg_x_movement = sum_x_movement/numpeaks
            #print(f'P num:{numpeaks} avg_dist:{avg_x_distance} sum_x:{sum_x_movement} avg_x:{avg_x_movement}')
            ...
            for i in range(len(active_ids_sorted)):
                ...
                y_prev = y
                #print(f'p:{peak_id}, x:{x}, y:{y}, xmov:{x_movement} amp {peak_amp}')
