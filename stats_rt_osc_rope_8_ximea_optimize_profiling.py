...
fps = 30
flip = False
mask_up_left  = (0.09, 0.15)
mask_up_right = (0.95, 0.25)
mask_lo_right = (0.95, 0.9)
mask_lo_left  = (0.09, 0.95)
mask_left = int(mask_up_left[0]*dimensions[1]) # left top, assumes vertical left edge
mask_right = int(mask_up_right[0]*dimensions[1]) # right top, as above
mask_xrange = mask_right-mask_left
...
mask_center = np.linspace(wavecenter_y_left, wavecenter_y_right, mask_xrange)
send_counter = 0
max_numpeaks = 0
...
current_peak_id = 0
peak_max_change = mask_xrange*0.3
prev_wave_1D = np.zeros(mask_xrange)
...
prev_faders = np.zeros(num_faders)
...
show_binary = False#True
show_centroid = True#False
show_fill_blanks = True#False
...
def find_center_wave_regr(wave_1D, mask_left, mask_right):
    x = np.arange(0,len(wave_1D),1)
    m, c = np.polyfit(x, wave_1D, 1)
    regr = np.linspace(c,c+len(wave_1D)*m,len(wave_1D))
...
def centroid_1D_from_img(input_img, output_img, show_centroid, centroid_color):
    # centroid
    centroid_1D = np.zeros(mask_xrange)
    for i in range(mask_xrange):
        y_coords = np.nonzero(input_img[:,i+mask_left])
        if len(y_coords[0]) > 0:
            y_centroid = np.mean(y_coords)
            y_centroid = int(np.round(y_centroid))
            centroid_1D[i] = y_centroid
            if show_centroid:
                cv2.circle(output_img, (i+mask_left,y_centroid),5, centroid_color, 1)# display centroid
    return centroid_1D
...
def fill_in_missing_points(y_init, input_1D, output_1D, output_img, show_fill_blanks, fill_blanks_color):
    ...
    line_len = np.min((mask_xrange-x_last, int(mask_xrange/10)))
    line = np.linspace(y_prev, y_init, line_len)
    output_1D[x_last:x_last+line_len] = np.reshape(line, shape=(line_len,))
    if show_fill_blanks:
        for i in range(mask_xrange):
            cv2.circle(output_img, (i+mask_left,int(output_1D[i])), 3, fill_blanks_color, 1)
    return output_1D

def lowpass_1D(input_1D, center_wave, output_img, show_lowpassfilter, lowpass_color):
    
    input_1D -= center_wave # need to be centered around zero for lowpass
    input_1D = lfilter(b, a, input_1D)
    input_1D = np.flip(input_1D)
    input_1D = lfilter(b, a, input_1D)
    input_1D = np.flip(input_1D)
    input_1D += center_wave
    if show_lowpassfilter:
        for i in range(mask_xrange):
            cv2.circle(output_img, (i+mask_left,int(input_1D[i])), 4, lowpass_color, 1)
    return input_1D

def find_peaks(input_1D, center_wave, show_wavesign, wavesign_color, show_wavecenter, wavecenter_color):
    ...
    for i in range(mask_xrange):
        if show_wavesign:
            ...
            cv2.circle(wave_img, (i+mask_left,y), 2, wavesign_color, 1)# display sign
        if show_wavecenter:
            cv2.circle(wave_img, (i+mask_left,int(center_wave[i])), 1, wavecenter_color, 1)# display wave_center
    sign_indices = []
    signum_old = 0
    for i in range(mask_xrange):
        signum = sign[i]
...
def display_peaks(active_ids, center_wave, input_1D, output_img, show_peaks, peakplus_color, peaknegative_color):
    for p in active_ids:
        ...
        if show_peaks:
            if y > center_wave[x]:
                cv2.circle(output_img, (x+mask_left,y),10, peakplus_color, 4)
                cv2.putText(output_img, f'{peak_id}', (x+mask_left-20,y+45), cv2.FONT_HERSHEY_SIMPLEX, 1, peakplus_color, 2, cv2.LINE_AA)
            else:
                cv2.circle(output_img, (x+mask_left,y),10, peaknegative_color, 4)
                cv2.putText(output_img, f'{peak_id}', (x+mask_left-20,y-25), cv2.FONT_HERSHEY_SIMPLEX, 1, peaknegative_color, 2, cv2.LINE_AA) 
...
try:
    ...
    while True:
        ...
        noise_gate = 1
        #lowpass_coef = 0.95
        #binary_img = ((binary_img*lowpass_coef)+(prev_binary_img*(1-lowpass_coef))).astype('uint8')
        ...
        # filter padding of wave_1D
        filter_padding = 20
        wave_1D = np.zeros(mask_xrange+(filter_padding*2))+wavecenter_y_left
        ...
        # lowpass
        wave_1D = lowpass_1D(wave_1D, center_wave, wave_img, show_lowpassfilter, lowpass_color)
        ...
        if noise_gate > 0:
            peak_indices = find_peaks(wave_1D, center_wave, show_wavesign, wavesign_color, show_wavecenter, wavecenter_color)
        else:
            peak_indices = []
        ...
        # grid faders
        fader_pad = 0#int(((mask_xrange)/(num_faders))*0.2)#int((dimensions[1]/(num_faders))*0.2)
        fader_distance = int(((mask_xrange)-fader_pad*2)/(num_faders-1))
        for i in range(num_faders):
            #faders[i] = wave_1D[i*fader_distance+fader_pad+mask_left]#/max_amp
            faders[i] = wave_1D[i*int(mask_xrange/(num_faders-1))]#/max_amp
        # need better fader filter: moving average with attack and decay time
        #fader_lp_coef = 0.2
        #faders = (faders*fader_lp_coef) + (prev_faders*(1-fader_lp_coef))
        #x = (i*fader_distance)+mask_left+fader_pad
        display_faders(faders, num_faders, fader_distance, fader_pad, mask_left, mask_center, max_amp, wave_img, show_faders, fader_color)
        prev_faders = np.copy(faders)
        ...
        if numpeaks > 0 :
            ...
            # normalize
            avg_x_movement /= (mask_xrange)
            avg_x_distance /= (mask_xrange)
            # send
